...
def evaluate_metric(model, data_iter, scaler):
    ...
    with torch.no_grad():
        mae, sum_y, mape, mse = [], [], [], []
        # 定义极小值，避免除以零（可根据数据量级调整，1e-8适合大多数场景）
        eps = 1e-8

        for x, y in data_iter:
            # 反归一化：转为numpy并展平（保持原逻辑）
            y = scaler.inverse_transform(y.cpu().numpy()).reshape(-1)
            y_pred = scaler.inverse_transform(model(x).view(len(x), -1).cpu().numpy()).reshape(-1)
            ...
            # 累加各类指标（修复核心：处理y=0的情况）
            mae += d.tolist()
            sum_y += y.tolist()
            # 修复1：给y加eps避免除以零，同时用np.abs(y)防止y为负数的情况
            mape += (d / (np.abs(y) + eps)).tolist()
            mse += (d ** 2).tolist()

        # 计算最终指标（保持原逻辑）
        MAE = np.array(mae).mean()
        RMSE = np.sqrt(np.array(mse).mean())
        WMAPE = np.sum(np.array(mae)) / (np.sum(np.array(sum_y)) + eps)  # 修复2：防止sum_y总和为0

        # 可选：如果需要返回MAPE，取消注释（已修复除以零问题）
        # MAPE = np.array(mape).mean()
