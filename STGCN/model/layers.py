...
class Align(nn.Module):
    def __init__(self, c_in, c_out):
        ...
        self.c_out = c_out  # 目标输出通道数
        # 定义1x1卷积层：仅改变通道数，不改变空间/时间维度（kernel_size=(1,1)）
        self.align_conv = nn.Conv2d(in_channels=c_in, out_channels=c_out, kernel_size=(1, 1))

    def forward(self, x):
        # 情况1：输入通道数 > 输出通道数 → 用1x1卷积降维
        if self.c_in > self.c_out:
            x = self.align_conv(x)
        # 情况2：输入通道数 < 输出通道数 → 补零升维
        elif self.c_in < self.c_out:
            batch_size, _, timestep, n_vertex = x.shape  # 获取输入张量的维度
            # 构造补零的张量，维度为[批次, 需补充的通道数, 时间步, 顶点数]，并放到和x相同的设备（CPU/GPU）
            pad_tensor = torch.zeros([batch_size, self.c_out - self.c_in, timestep, n_vertex]).to(x)
            # 在通道维度（dim=1）拼接，实现通道数扩充
            x = torch.cat([x, pad_tensor], dim=1)
        # 情况3：输入通道数 = 输出通道数 → 直接返回，不做处理
        else:
            x = x
        return x
...
class CausalConv1d(nn.Conv1d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, enable_padding=False, dilation=1, groups=1,
        ...
        if enable_padding == True:
            # 核心公式：因果padding = (卷积核大小 - 1) * 膨胀系数
            self.__padding = (kernel_size - 1) * dilation
        else:
            self.__padding = 0
        # 调用父类nn.Conv1d的初始化，传入计算好的padding
    ...
    def forward(self, input):
        # 调用父类nn.Conv1d的forward方法，执行普通一维卷积（带padding）
        result = super(CausalConv1d, self).forward(input)
        # 如果padding不为0，截断最后self.__padding个时间步（消除padding带来的长度冗余）
        if self.__padding != 0:
