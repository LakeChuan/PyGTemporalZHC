...
class SpatialConvLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, K=3):
        super().__init__()
        self.cheb_conv = ChebConv(in_channels, out_channels, K=K)
        self.relu = nn.ReLU()

    def forward(self, x, edge_index, edge_weight):
        # x: [B*T, N, C]
        return self.relu(self.cheb_conv(x, edge_index, edge_weight))
...
class STGCNBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.temporal = TemporalConvLayer(in_channels, out_channels)
        self.spatial = SpatialConvLayer(out_channels, out_channels)
...
class STGCN(nn.Module):
    ...
    def __init__(self, args):
        ...
        # 两层时空卷积块
        self.st_block1 = STGCNBlock(5, 64)
        self.st_block2 = STGCNBlock(64, 64)
    ...
    def forward(self, x, edge_index, edge_weight):
        # 时空特征提取
        x = self.st_block1(x, edge_index, edge_weight)
        x = self.st_block2(x, edge_index, edge_weight)
...
def plot_result_with_diff(y, pred, args):  # 加入 args
    ...
    ax1 = plt.subplot(131);
    im1 = ax1.imshow(y, cmap='Blues', vmin=vmin, vmax=vmax);
    ax1.set_title("真实水深");
    ax1.axis("off");
    plt.colorbar(im1, ax=ax1, shrink=0.75)
    ax2 = plt.subplot(132);
    im2 = ax2.imshow(p, cmap='Blues', vmin=vmin, vmax=vmax);
    ax2.set_title("预测水深");
    ax2.axis("off");
    plt.colorbar(im2, ax=ax2, shrink=0.75)
    ax3 = plt.subplot(133);
    im3 = ax3.imshow(diff, cmap='RdBu_r', norm=diff_norm);
    ax3.set_title("预测误差");
    ax3.axis("off");
...
def run(args):
    ...
    save_dir = f"run_result_{args.modelname}/{timestamp}_STGCN_Epoch{args.epochs}"
    ...
    for epoch in range(args.epochs):
        ...
        if val_rmse < best_rmse:
            best_rmse = val_rmse
            torch.save(model.state_dict(), os.path.join(save_dir, f"best_stgcn.pth"))
    ...
    # 测试
    model.load_state_dict(torch.load(os.path.join(save_dir, "best_stgcn.pth")))
    model.eval()
    print("\n🎉 测试集结果：")
...
def parse_args():
    ...
    # 模型参数
    parser.add_argument('--modelname', type=str, default="STGCN", help='模型名字STGCN')
